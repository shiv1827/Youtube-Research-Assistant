...
async def get_insights_and_stories(chunk: str) -> tuple[List[str], List[str], List[str]]:
    """Extract insights, stories, and thoughts using GPT-4."""
    prompt = f"""Analyze this video transcript chunk and extract:
    1. Key insights (practical takeaways or lessons)
    2. Stories or examples
    3. Interesting thoughts or reflections
    ...
    return insights, stories, thoughts
...
async def process_chunk(chunk: Dict[str, Any], chunk_number: int, video_id: str) -> ProcessedVideoChunk:
    ...
    insights, stories, thoughts = await get_insights_and_stories(chunk["text"])
